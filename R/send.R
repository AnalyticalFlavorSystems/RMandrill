...
#' @param images character vector denoting location of PNGs to include in email. PNG file names (minus the suffix and
#' any preceeding path directories) must match the image declaration in the Mandrill template.
#' @param attachments character vector denoting location of attachments to include in email.
...
mandrill_send_template <- function(api_key = NA,
  ...
  if (!missing(attachments)) {
    attachments_out <- dplyr::rbind_all(lapply(attachments,
                                               process_attachments
                                               )
                                        )
  }
...
.post <- function(link, data) {
  res <- httr::POST(url = link,
                    httr::user_agent(agent = "Mandrill-Curl/1.0"),
                    body = data
                    )
  httr::content(res)
}
